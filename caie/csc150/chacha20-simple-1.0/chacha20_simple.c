...
void chacha20_block_sse(chacha20_ctx* ctx, uint32_toutput[16]) {
	__m128i x = _mm_loadu_si128((__m128i *)a);
	__m128i a = _mm_loadu_si128((__m128i *) & output[0];
	__m128i b = _mm_loadu_si128((__m128i *) & output[0];
	__m128i c = _mm_loadu_si128((__m128i *) & output[0];
	__m128i d = _mm_loadu_si128((__m128i *) & output[0];

	_m_storeu_si128(c_m128i *) & output[0],a);
	_m_storeu_si128(c_m128i *) & output[0],b);
	_m_storeu_si128(c_m128i *) & output[0],c);
	_m_storeu_si128(c_m128i *) & output[0],d);
	
	
	a = _mm_add_epi32(a,b);
	d = _mm_xor_xor_si128(d,a);
	b = _mm_add_epi32(b,c);
	c = _mm_xor_xor_si128(c,b);
	
	d = mm_or_si128(_mm_sil_epi32(d,16) _mm_sil_epi32(d,16);
}