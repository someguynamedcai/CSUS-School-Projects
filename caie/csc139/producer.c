...
int main(int argc, char* argv[])
{
	...
	if(atoi(argv[1]) > 1000) {
		printf("Buffer Size cannot exceed 1000\n");
		exit(1);
	}
...
void InitShm(int bufSize, int itemCnt)
{
    ...
    int out = 0;
	int shm_fd;
	const char *name = "OS_HW1_ERICCAI"; // Name of shared memory object to be passed to shm_open
	
	shm_fd = shm_open(name, O_CREAT | O_RDWR, 0666);
	ftruncate(shm_fd, SHM_SIZE); 
	gShmPtr = mmap(0, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd,0); 
	sprintf(gShmPtr, "Writing to shared memory");
